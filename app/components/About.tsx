    ...
    <section id="about" className="max-w-7xl mx-auto px-6 lg:px-12 py-28">
      <div className="grid lg:grid-cols-2 gap-20 items-start">
        ...
        <div>
          ...
          <h2 className="font-display font-light leading-[1.08] mb-6" style={{ fontSize: "clamp(38px, 4.5vw, 60px)", color: "var(--near-black)" }}>
            A Sanctuary of<br /><em style={{ color: "var(--deep-purple)" }}>Hope & Strength</em>
          </h2>
          ...
          <div className="grid sm:grid-cols-2 gap-5 mb-10">
            {[
              ...
              <div key={c.tag} className="rounded-xl p-6" style={{ background: "var(--light-blush)", borderLeft: "3px solid var(--deep-purple)" }}>
          ...
          <div className="flex items-center gap-5 p-6 rounded-2xl" style={{ background: "var(--near-black)", border: "1px solid rgba(244,167,195,0.12)" }}>
        ...
        {/* Right — Photo + Values */}
        <div>
          {/* Smiling African woman — verified photo-1509099863731-ef4bff19e808 */}
          <div className="relative overflow-hidden rounded-2xl mb-8" style={{ height: "300px" }}>
            <img
              src="https://images.unsplash.com/photo-1509099863731-ef4bff19e808?w=900&q=85&auto=format&fit=crop"
              alt="African woman smiling — SMCO community member"
              className="w-full h-full object-cover"
              style={{ objectPosition: "center 20%" }}
            />
            <div className="absolute inset-0" style={{ background: "linear-gradient(to top, rgba(61,26,94,0.6) 0%, transparent 50%)" }} />
            <div className="absolute bottom-5 left-5 right-5 flex items-center justify-between">
              <span className="text-white text-sm font-semibold">Kampala, Uganda</span>
              <span className="text-xs px-3 py-1 rounded-full font-semibold" style={{ background: "var(--accent-pink)", color: "#fff" }}>Est. 2025</span>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-3">
            {values.map((v) => (
              <div key={v.label} className="flex items-center gap-4 px-5 py-4 rounded-xl cursor-default transition-all duration-200" style={{ background: "#fff", border: "1px solid rgba(192,68,106,0.1)" }}
                onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.borderColor = "var(--deep-purple)"; (e.currentTarget as HTMLElement).style.transform = "translateX(6px)"; }}
                onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.borderColor = "rgba(192,68,106,0.1)"; (e.currentTarget as HTMLElement).style.transform = "translateX(0)"; }}>
